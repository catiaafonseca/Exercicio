...
if(isset($_POST["product_code"]))
{
	...
	$statement = $mysqli->prepare("SELECT nome, preco, descricao FROM produto WHERE codigo=? LIMIT 1");
	$statement->bind_param('s', $new_product['product_code']);
	$statement->execute();
	$statement->bind_result($product_name, $product_price, $product_description);
...
if(isset($_POST["load_cart"]) && $_POST["load_cart"]==1)
{
	...
	if(isset($_SESSION["products"]) && count($_SESSION["products"])>0){ //if we have session variable
		...
		foreach($_SESSION["products"] as $product){ //loop though items and prepare html content
			...
			$cart_box .=  "<li> $product_name $product_description (Quantidade : $product_qty | $product_size ) - ".$product_price * $product_qty. "€ <a href=\"#\" class=\"remove-item\" data-code=\"$product_code\">&times;</a></li>";
			$subtotal = ($product_price * $product_qty);
			$total = ($total + $subtotal);
		}
		$cart_box .= "</ul>";
		$cart_box .= '<div class="cart-products-total">Total : '.$total.'€ <u><a href="?p=view_cart" title="Verifica o carrinho e faz checkout">Checkout</a></u></div>';
